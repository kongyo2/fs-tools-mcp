...
export async function readPDF(filePath: string): Promise<PDFResult<{ type: "pdf"; file: { filePath: string; base64: string; originalSize: number } }>> {
  try {
    ...
    return { success: false, error: { reason: "unknown", message: errorMessage(error) } };
  }
}

export async function getPDFPageCount(filePath: string): Promise<number | null> {
  const result = await execFileNoThrow("pdfinfo", [filePath], { timeout: 10_000 });
  if (result.code !== 0) {
    return null;
  }
...
): Promise<PDFResult<{ type: "parts"; file: { filePath: string; originalSize: number; count: number; outputDir: string } }>> {
  try {
    const fileBuffer = await readFile(filePath);
    const originalSize = fileBuffer.length;
    if (originalSize === 0) {
    ...
    if (!(await isPdftoppmAvailable())) {
      return {
        ...
        error: {
          reason: "unavailable",
          message: "pdftoppm is not installed. Install poppler-utils to enable PDF page rendering."
        }
      };
    }
    ...
    const result = await execFileNoThrow("pdftoppm", args, { timeout: 120_000 });
    if (result.code !== 0) {
      ...
      return { success: false, error: { reason: "unknown", message: `pdftoppm failed: ${result.stderr}` } };
    }
    const imageFiles = (await readdir(outputDir)).filter((entry) => entry.endsWith(".jpg")).sort();
