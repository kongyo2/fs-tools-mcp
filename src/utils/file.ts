import { chmodSync, existsSync, readFileSync, renameSync, readdirSync, statSync, unlinkSync, writeFileSync } from "node:fs";
...
export function writeTextContent(filePath: string, content: string, encoding: BufferEncoding, endings: LineEndingType): void {
  let toWrite = content;
  if (endings === "CRLF") {
...
export function writeFileSyncAndFlush(filePath: string, content: string, options: { encoding: BufferEncoding; mode?: number }): void {
  ...
  try {
    writeFileSync(tempPath, content, { encoding: options.encoding, flush: true, ...(targetExists ? {} : options.mode !== undefined ? { mode: options.mode } : {}) });
...
export function findSimilarFile(filePath: string): string | undefined {
  try {
    ...
    const fileBaseName = filePath.slice(filePath.lastIndexOf(sep) + 1, filePath.lastIndexOf(extname(filePath)));
    const files = readdirSync(dir, { withFileTypes: true });
    const match = files.find((entry) => entry.isFile() && entry.name !== filePath && entry.name.slice(0, entry.name.lastIndexOf(extname(entry.name))) === fileBaseName);
    return match?.name;
  } catch (error) {
    if (!isENOENT(error)) {
      return undefined;
...
export async function suggestPathUnderCwd(requestedPath: string): Promise<string | undefined> {
  ...
  if (!requestedPath.startsWith(prefix) || requestedPath === cwd || requestedPath.startsWith(`${cwd}${sep}`)) {
    return undefined;
  }
...
export function addLineNumbers(file: { content: string; startLine: number }): string {
  if (!file.content) {
    return "";
  }
  const lines = file.content.split(/\r?\n/);
  return lines.map((line, index) => `${index + file.startLine}\t${line}`).join("\n");
}
...
export function formatReadTooLargeMessage(sizeInBytes: number, maxSizeBytes: number): string {
