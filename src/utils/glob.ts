...
export function extractGlobBaseDirectory(pattern: string): { baseDir: string; relativePattern: string } {
  ...
  const lastSeparatorIndex = Math.max(staticPrefix.lastIndexOf("/"), staticPrefix.lastIndexOf(sep));
...
export async function glob(
  ...
  const noIgnore = (process.env.FS_TOOLS_MCP_GLOB_NO_IGNORE ?? "true").toLowerCase() !== "false";
  const hidden = (process.env.FS_TOOLS_MCP_GLOB_HIDDEN ?? "true").toLowerCase() !== "false";
  const args = [
    "--files",
    "--glob",
    searchPattern,
  ...
  const absolutePaths = allPaths.map((path) => (isAbsolute(path) ? path : join(searchDir, path)));
